...
function getInventoryRowValues(productId) {
  ...
  if (!Number.isFinite(stockQuantity) || stockQuantity < 0 || !Number.isFinite(lowStockThreshold) || lowStockThreshold < 0) throw new Error("Stock and low stock threshold must be non-negative numbers.");
...
async function saveAllVisibleInventory() {
  ...
  const pending = [];
  const invalid = [];
  getAllProducts().filter(productMatchesInventorySearch).forEach((product) => {
    try { pending.push({ productId: product.id, values: getInventoryRowValues(product.id) }); } catch (error) { invalid.push(error); }
  });
  if (invalid.length) throw new Error(`${invalid.length} row${invalid.length === 1 ? " has" : "s have"} invalid stock values. Fix them before saving.`);
  ...
  if (failed.length) throw new Error(`${failed.length} of ${pending.length} inventory items could not be saved.`);
